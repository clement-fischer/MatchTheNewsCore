...
public class DocumentImporter extends Importer {
	...
	public void importData(boolean asGraphOfWords) {
		...
		tpcRelImp.usedFiles.forEach(f -> {
			...
			Boolean isEnglish = filePath.toString().endsWith("ENG.tkn_sgm")
					&& (SystemProperties.READ_ONLY_ENGLISH_DOCUMENTS);
			...
			if ((Files.isRegularFile(filePath)) && toRead) {
				...
				for (Element d : all_docs) {
					if (!tpcRelImp.usedDocsIds.contains(d.select("docno").text()))
						continue;
					documents_tkn.put(d.select("docno").text(), new TDTDocument(d.select("docno").text(),
							d.select("text").text(), asGraphOfWords, docs_topics.get(d.select("docno").text())));
				}
			}
		});
		...
		for (Entry<String, TDTDocument> e : set) {
			if (tpcRelImp.usedTopics.contains(e.getValue().getLabel())) {
				...
				e.getValue().setIsFirstStory(false);
		}
